...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int x = 0; x < solution.length; x++) {
            for (int y = 0; y < solution[x].length; y++) {
                int k = solution[x][y];
                if (k == 1 || k == last) {
                    Sq sq1 = new Sq(x, y, k, true, arrowDirection(x, y), 0);
                    _board[x][y] = sq1;
                } else {
                    Sq sq1 = new Sq(x, y, 0, false, arrowDirection(x, y), -1);
                    _board[x][y] = sq1;
                }
                _allSquares.add(_board[x][y]);
                _solnNumToPlace[k] = Place.pl(x, y);
            }
        }

        for (int i = 0; i < _allSquares.size(); i++) {
            if (_allSquares.get(i) == null) {
                throw badArgs("All numbers from 1 - last number must appear");
            }
        }
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                Sq sq1 = get(x, y);
                PlaceList [][][] suc = Place.successorCells(_width, _height);
                PlaceList l1 = suc[x][y][sq1.direction()];
                sq1._successors = new PlaceList();
                for (int i = 0; i < l1.size(); i++) {
                    Place p1 = l1.get(i);
                    sq1._successors.add(p1);
                    Sq target = get(p1.x, p1.y);
                    if (target._predecessors == null) {
                        target._predecessors = new PlaceList();
                    }
                    target._predecessors.add(pl(x, y));
                }
            }
        }
        _unconnected = last - 1;
    ...
    Model(Model model) {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                ...
                this._allSquares.add(_board[x][y]);
            }
        }

        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                Sq sq1 = model.get(x, y);
                _board[x][y]._successor = get(sq1.successor());
                _board[x][y]._predecessor = get(sq1.predecessor());
                _board[x][y]._head = get(sq1.head());
            }
        }
    }
    ...
    boolean autoconnect() {
        ...
        for (int i = 1; i < _solnNumToPlace.length - 1; i++) {
            Sq sq1 = solnNumToSq(i);
            boolean nextExists = solnNumToSq(i + 1) != null;
            if (sq1 != null && sq1.sequenceNum() != 0 && nextExists) {
                Sq sq2 = solnNumToSq(i + 1);
                if (sq2.sequenceNum() != 0) {
                    result = sq1.connect(solnNumToSq(i + 1));
                }
            }
        }
        return result;
    ...
    void solve() {
        for (int i = 1; i < _solnNumToPlace.length - 1; i++) {
            solnNumToSq(i)._sequenceNum = i;
            solnNumToSq(i).connect(solnNumToSq(i + 1));
        }
        solnNumToSq(_width * _height)._sequenceNum = _width * _height;
        _unconnected = 0;
    }
    ...
    public int arrowDirection(int x, int y) {
        ...
        for (int x1 = 0; x1 < _solution.length; x1++) {
            for (int y1 = 0; y1 <  _solution[x1].length; y1++) {
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            boolean dirCheck = Place.dirOf(x, y, s1.x, s1.y) == _dir;
            boolean predSuc = s1.predecessor() == null && successor() == null;
            boolean notFirst = s1.sequenceNum() != 1;
            boolean notLast = sequenceNum() != _width * _height;
            if (dirCheck && predSuc && notFirst && notLast) {
                if (s1.sequenceNum() != 0 && sequenceNum() != 0) {
                    return sequenceNum() == s1.sequenceNum() - 1;
                } else if (s1.sequenceNum() == 0 && sequenceNum() == 0) {
                    return s1.group() != group() || s1.group() == -1;
                }
                return true;
            }
            return false;
        ...
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() != 0) {
                ...
                int i = 1;
                while (pred != null) {
                    pred._sequenceNum = this.sequenceNum() - i;
                    i++;
            ...
            Sq succ = this._successor;
            int i = 1;
            while (succ != null) {
                succ._head = this.head();
                if (setsuc) {
                    succ._sequenceNum = this.sequenceNum() + i;
                }
                i++;
                succ = succ.successor();
            }
        ...
        void disconnect() {
            ...
            if (!fixed[1]) {
                suc = next;
                if (next.successor() != null && this.predecessor() != null) {
                    newGroups[1] = newGroup();
                }
                while (suc != null) {
                    if (newGroups[1] != 0) {
                        suc._group = newGroups[1];
                    }
                    suc._sequenceNum = 0;
                    suc = suc.successor(); }
            } else if (!fixed[0]) {
