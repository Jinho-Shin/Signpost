...
import static signpost.Place.successorCells;
...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        PlaceList [][][] suc = successorCells(model.width(), model.height());
        PlaceList successors = suc[start.x][start.y][start.direction()];
        int count = 0;
        Sq sq1 = null;
        for (int i = 0; i < successors.size(); i++) {
            Sq sqi = model.get(successors.get(i));
            if (start.connectable(sqi)) {
                count++;
                if (count > 1) {
                    break;
                }
        ...
        if (start.sequenceNum() != 0) {
            Sq sq2 = model.solnNumToSq(start.sequenceNum() + 1);
            if (sq2.sequenceNum() != 0 && start.connectable(sq2)) {
                return sq2;
            }
        }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        PlaceList [][][] pred = successorCells(model.width(), model.height());
        PlaceList predecessors = pred[end.x][end.y][0];
        int count = 0;
        Sq sq1 = null;
        for (int i = 0; i < predecessors.size(); i++) {
            Sq sqi = model.get(predecessors.get(i));
            if (sqi.connectable(end)) {
                count++;
                if (count > 1) {
                    break;
                }
